...
public class Text2Docs {

	private static Logger logger = MalletLogger.getLogger(Text2Docs.class.getName());
	public static String defaultTokenRegex = "\\p{L}[\\p{L}\\p{P}]+\\p{L}";

	static CommandOption.SpacedStrings classDirs =	new CommandOption.SpacedStrings(Text2Docs.class, "input", "DIR...", true, null,
		 "The directories containing text files to be classified, one directory per class", null);
	
	static CommandOption.File outputFile = new CommandOption.File(Text2Docs.class, "output", "FILE", true, new File("text.vectors"),
		 "Write the instance list to this file; Using - indicates stdout.", null);
	
	static CommandOption.File usePipeFromVectorsFile = new CommandOption.File(Text2Docs.class, "use-pipe-from", "FILE", true, new File("text.vectors"),
	...
	static CommandOption.Boolean preserveCase = new CommandOption.Boolean(Text2Docs.class, "preserve-case", "[TRUE|FALSE]", false, false,
		 "If true, do not force all strings to lowercase.", null);
	
    static CommandOption.SpacedStrings replacementFiles = new CommandOption.SpacedStrings(Text2Docs.class, "replacement-files", "FILE [FILE ...]", true, null,
             "files containing string replacements, one per line:\n    'A B [tab] C' replaces A B with C,\n    'A B' replaces A B with A_B", null);

    static CommandOption.SpacedStrings deletionFiles = new CommandOption.SpacedStrings(Text2Docs.class, "deletion-files", "FILE [FILE ...]", true, null,
             "files containing strings to delete after replacements but before tokenization (ie multiword stop terms)", null);

	static CommandOption.Boolean removeStopWords = new CommandOption.Boolean(Text2Docs.class, "remove-stopwords", "[TRUE|FALSE]", false, false,
		 "If true, remove a default list of common English \"stop words\" from the text.", null);

	static CommandOption.File stoplistFile = new CommandOption.File(Text2Docs.class, "stoplist-file", "FILE", true, null,
		 "Instead of the default list, read stop words from a file, one per line. Implies --remove-stopwords", null);

	static CommandOption.File extraStopwordsFile = new CommandOption.File(Text2Docs.class, "extra-stopwords", "FILE", true, null,
	...
	static CommandOption.File stopPatternFile = new CommandOption.File(Text2Docs.class, "stop-pattern-file", "FILE", true, null,
		 "Read regular expressions from a file, one per line. Tokens matching these regexps will be removed.", null);

	static CommandOption.Boolean skipHeader = new CommandOption.Boolean(Text2Docs.class, "skip-header", "[TRUE|FALSE]", false, false,
	...
	static CommandOption.Boolean skipHtml = new CommandOption.Boolean(Text2Docs.class, "skip-html", "[TRUE|FALSE]", false, false,
		 "If true, remove text occurring inside <...>, as in HTML or SGML.", null);
	
	static CommandOption.Boolean binaryFeatures = new CommandOption.Boolean(Text2Docs.class, "binary-features", "[TRUE|FALSE]", false, false,
		 "If true, features will be binary.", null);
	
	static CommandOption.IntegerArray gramSizes = new CommandOption.IntegerArray(Text2Docs.class, "gram-sizes", "INTEGER,[INTEGER,...]", true, new int[] {1},
	...
	static CommandOption.Boolean keepSequence = new CommandOption.Boolean(Text2Docs.class, "keep-sequence", "[TRUE|FALSE]", false, false,
		 "If true, final data will be a FeatureSequence rather than a FeatureVector.", null);

	static CommandOption.Boolean keepSequenceBigrams = new CommandOption.Boolean(Text2Docs.class, "keep-sequence-bigrams", "[TRUE|FALSE]", false, false,
		 "If true, final data will be a FeatureSequenceWithBigrams rather than a FeatureVector.", null);
        
        static CommandOption.Boolean readSocialText = new CommandOption.Boolean(Text2Docs.class, "keep-sentences", "[TRUE|FALSE]", false, false,
		 "If true, final data will be a FeatureSequenceVector rather than a FeatureVector.", null);
	
	static CommandOption.Boolean saveTextInSource = new CommandOption.Boolean(Text2Docs.class, "save-text-in-source", "[TRUE|FALSE]", false, false,
		 "If true, save original text of document in source.", null);
	
	static CommandOption.ObjectFromBean stringPipe = new CommandOption.ObjectFromBean(Text2Docs.class, "string-pipe", "Pipe constructor",	true, null,
		 "Java code for the constructor of a Pipe to be run as soon as input becomes a CharSequence", null);
	
	static CommandOption.ObjectFromBean tokenPipe = new CommandOption.ObjectFromBean(Text2Docs.class, "token-pipe", "Pipe constructor",	true, null,
		 "Java code for the constructor of a Pipe to be run as soon as input becomes a TokenSequence", null);
	
	static CommandOption.ObjectFromBean featureVectorPipe = new CommandOption.ObjectFromBean(Text2Docs.class, "fv-pipe", "Pipe constructor",	true, null,
		 "Java code for the constructor of a Pipe to be run as soon as input becomes a FeatureVector", null);
	
	static CommandOption.String encoding = new CommandOption.String(Text2Docs.class, "encoding", "STRING", true, Charset.defaultCharset().displayName(),
		 "Character encoding for input file", null);
	
	static CommandOption.String tokenRegex = new CommandOption.String(Text2Docs.class, "token-regex", "REGEX", true, defaultTokenRegex,
	...
	static CommandOption.Boolean printOutput = new CommandOption.Boolean(Text2Docs.class, "print-output", "[TRUE|FALSE]", false, false,
		...
		if (usePipeFromVectorsFile.wasInvoked()) {
			...
			if (! preserveCase.value()) {
				pipeList.add(new CharSequenceLowercase());
                        ...
                        if(readSocialText.wasInvoked()){
                          //  pipeList.add(new CharacterSequence2SocialText());
                        }
...
/*			if (! (keepSequence.value || keepSequenceBigrams.value||keepSentences.value)) {
